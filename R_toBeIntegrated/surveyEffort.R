...
garDF <- df %>%
  ...
  dplyr::select(track_fid, track_seg_id, track_seg_point_id,
                source, dateTime_GMT, dateTime_local, geometry, gpxtpx_TrackPointExtension) %>%
  mutate(longitude = sf::st_coordinates(.)[, 1],
         latitude = sf::st_coordinates(.)[, 2]) %>% st_drop_geometry()

# Combining the two track logs
oDF <- clDF %>%
  dplyr::select(survey, source, datetime_gmt = dateTime_GMT, datetime_local = dateTime_local,
                longitude = Longitude, latitude = Latitude) %>%
  add_row(garDF %>% dplyr::select(survey = track_seg_id, source,
                                 datetime_gmt = dateTime_GMT, datetime_local = dateTime_local,
                                 longitude, latitude))

clDates <- unique(oDF %>% filter(!grepl("Garmin", source)) %>% pull(datetime_local) %>% as.Date() %>% as.character())
garminDates <- unique(oDF %>% filter(grepl("Garmin", source)) %>% pull(datetime_local) %>% as.Date() %>% as.character())
...
oPts <- st_as_sf(oDF2, coords = c("longitude", "latitude"),
                 crs = 4326)
oLines <- oPts %>%
  arrange(datetime_local) %>%
  group_by(surveyid) %>%
  summarize(date = as.Date(datetime_local[1]), do_union=F,
            .group = "keep") %>%
...
ommDF2023 <- read.csv("E:/2024_PNW_data/Whale_Observations_all_pre-mid2023.csv") %>%
...
mmDF2023 <- read.csv("E:/2024_PNW_data/MMVS_Observations_2023-11-01-17-38-35.csv") %>%
...
mmDF <- read.csv("E:/2024_PNW_data/MMVS_Observations_2024-10-15-03-19-23.csv") %>%
  dplyr::select(dateTime = Observation.time,
                Longitude = X_Record.your.current.location_longitude,
                Latitude = X_Record.your.current.location_latitude,
                species = Species.,
  ...
  add_trace(
    data = obsDF %>%
      mutate(species = factor(species,
                                 levels = c("Humpback Whale", "Gray Whale"))), # %>% filter(species == "Gray Whale"),
    x = ~Longitude, y = ~Latitude,
    color = ~species,
    marker = list(
      #color = ~as.factor(species),
...
obsDF2 <- mmDF %>%
  filter(!grepl("Albatross", otherSpecies) &
...
N_excl_WSD <- obsDF2 %>%
  filter(!grepl("White", otherSpecies) &
           !grepl("Pws", otherSpecies) &
           !grepl("white", otherSpecies)) %>%
  pull(count) %>% sum()
Ninds_gray <- obsDF2 %>% filter(species == "Gray Whale") %>% pull(count) %>% sum()
...
N_excl_WSD <- obsDF2 %>%
  ...
  pull(count) %>% sum()
Ninds_gray <- obsDF2 %>% filter(species == "Gray Whale") %>% pull(count) %>% sum()
(Ninds_hump <- obsDF2 %>% filter(species == "Humpback Whale") %>% pull(count) %>% sum())
...
#
# spLI %>% st_length() %>% sum()
spDF2023 <- st_read("C:/Users/peter.mahoney/Desktop/WhitmireReq_BOEM_OffshoreWind_2023/shp/NOAA_MML_CalCurr_SurveyTrack_points.shp")
spDF2023 <- spDF2023 %>%
  mutate(
...
spDF <- oDF2 %>%
  dplyr::select(
    survey = surveyid, dateTime = datetime_local, Longitude = longitude,
    Latitude = latitude
  ) %>%
  filter(!is.na(dateTime) & Longitude < 0 &
           lubridate::year(dateTime) == 2024) %>%
